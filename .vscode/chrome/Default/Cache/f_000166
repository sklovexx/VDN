(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/ddz/scripts/gameScene/gameingUI.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, 'fc5fbLb+LFG+rCIt1gYkSVX', 'gameingUI', __filename);
// ddz/scripts/gameScene/gameingUI.js
...
cc.Class({
    ...
    onLoad: function onLoad() {
        ...
        _mygolbal2.default.socket.onCanRobState(function (data) {
            console.log("onCanRobState" + JSON.stringify(data));
            //这里需要2个变量条件：自己是下一个抢地主，2发牌动画结束
            this.rob_player_accountid = data;
            if (data == _mygolbal2.default.playerData.accountID && this.fapai_end == true) {
                this.robUI.active = true;
                // this.qdzTimer();
            }
        }.bind(this));
        _mygolbal2.default.socket.oninterval(function (data) {
            var gameScene_script = this.node.parent.parent.getComponent("gameScene");
            gameScene_script.emitEventToPlayer("interval", data);
            if (data.accountid == _mygolbal2.default.playerData.accountID) {
                if (_mygolbal2.default.roomState == 6) {
        ...
        _mygolbal2.default.socket.onendInterval(function (data) {
            var gameScene_script = this.node.parent.parent.getComponent("gameScene");
        ...
        //监听服务器可以出牌消息
        _mygolbal2.default.socket.onCanChuCard(function (data) {
            ...
            if (data.accountid == _mygolbal2.default.playerData.accountID) {
                //先清理出牌区域
                this.clearOutZone(_mygolbal2.default.playerData.accountID);
                //先把自己出牌列表置空
                //this.choose_card_data=[]
                //显示可以出牌的UI
                this.playingUI_node.active = true;
        ...
        //监听服务器：其他玩家出牌消息
        _mygolbal2.default.socket.onOtherPlayerChuCard(function (data) {
            ...
            for (var i = 0; i < data.cards.length; i++) {
                ...
                card.getComponent("card").showCards(data.cards[i].card_data, _mygolbal2.default.playerData.accountID);
        ...
        //监听服务器：游戏结算消息
        _mygolbal2.default.socket.onGameResult(function (data) {
            var _this = this;
            ...
            data.forEach(function (e) {
                console.log(e);
                if (e.account == _mygolbal2.default.playerData.accountID) {
                    console.log(e.change);
                    if (e.change < 0) {
                        var panel = cc.instantiate(_this.failPanel);
                        var script = panel.getComponent('failPanel');
                        script.initData(data);
                        panel.parent = _this.node.parent;
                    } else {
                        var _panel = cc.instantiate(_this.winPanel);
                        var _script = _panel.getComponent('winPanel');
                        _script.initData(data);
                        _panel.parent = _this.node.parent;
                    }
                }
            });
        }.bind(this));
        ...
        this.node.on("show_bottom_card_event", function (data) {
            ...
            //如果自己地主，给加上三张底牌
            if (_mygolbal2.default.playerData.accountID == _mygolbal2.default.playerData.master_accountid) {
    ...
    _getCardForTouch: function _getCardForTouch(touch) {
        for (var i = this.cards_nods.length - 1; i >= 0; i--) {
            // 需要倒序
    ...
    _checkSelectCardReserve: function _checkSelectCardReserve(touchBegan, touchMoved) {
        ...
        if (p1 === touchMoved) {
            ...
            for (var _i = 0; _i < this.cards_nods.length; _i++) {
                if (!this.cards_nods[_i].getBoundingBox().intersects(rect)) {
                    this.cards_nods[_i].isChiose = false;
                    console.warn(true);
                    this.cards_nods[_i].opacity = 255;
                }
            }
        }
    },
    ...
    pushCard: function pushCard(data) {
        ...
        for (var i = 0; i < 17; i++) {
            ...
            card.getComponent("card").showCards(data[i], _mygolbal2.default.playerData.accountID);
            //存储牌的信息,用于后面发牌效果
    ...
    pushThreeCard: function pushThreeCard() {
        ...
        for (var i = 0; i < this.bottom_card_data.length; i++) {
            ...
            card.getComponent("card").showCards(this.bottom_card_data[i], _mygolbal2.default.playerData.accountID);
            card.active = true;
    ...
    destoryAllCard: function destoryAllCard() {
    ...
    onButtonClick: function onButtonClick(event, customData) {
        switch (customData) {
            case "btn_qiandz":
                ...
                _mygolbal2.default.socket.requestRobState(qian_state.qian);
                this.robUI.active = false;
                if (true) {
            ...
            case "btn_buqiandz":
                ...
                _mygolbal2.default.socket.requestRobState(qian_state.buqiang);
                this.robUI.active = false;
                if (true) {
            ...
            case "nopushcard":
                //不出牌
                _mygolbal2.default.socket.request_buchu_card([], null);
                this.playingUI_node.active = false;
                break;
            case "pushcard":
                //出牌
                ...
                _mygolbal2.default.socket.request_chu_card(this.choose_card_data, function (err, data) {
...
cc._RF.pop();
        ...
        //# sourceMappingURL=gameingUI.js.map
