...
dataFunc.getMyDate=function(time) {
    let oDate = parseInt(time/1000);
    let oHour = Math.floor(oDate / (60 * 60));
    let modulo = oDate % (60 * 60);
    //分钟
    let oMin = Math.floor(modulo / 60);
    //秒
    let oSen = modulo % 60;
    let oTime = dataFunc.addZero(oHour) +'小时'+dataFunc.addZero(oMin)+'分'+dataFunc.addZero(oSen)+'秒';
    return oTime;
};
