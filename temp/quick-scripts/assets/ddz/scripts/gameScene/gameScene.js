...
cc.Class({
    ...
    onLoad: function onLoad() {
        ...
        _mygolbal2.default.socket.onRoomChangeState(function (data) {
            ...
            this.roomstate = data;
            _mygolbal2.default.roomState = data;
            ...
            if (this.roomstate == 1) {
                ...
                _mygolbal2.default.playerData.rate = 1;
        ...
        //监听玩家退出房间事件
        _mygolbal2.default.socket.onQuitPlayer(function (data) {
        ...
        //注册监听服务器倍数更新消息
        _mygolbal2.default.socket.onBeiShuUpdate(function (data) {
            console.log("onBeiShuUpdate----------" + data);
            console.log(_mygolbal2.default.playerData.rate);
            this.beishu_label.string = "倍数:" + data;
            _mygolbal2.default.playerData.rate = data;
        }.bind(this));
    },
    ...
    removePlayerNode: function removePlayerNode(player_data) {
        var _this = this;

        this.playerNodeList.forEach(function (e, i) {
    ...
    addPlayerNode: function addPlayerNode(player_data) {
        ...
        _mygolbal2.default.allPlayerData.push(player_data);
        console.log(this.node.getChildByName('container').children);
    },
    emitEventToPlayer: function emitEventToPlayer(event, data) {
        for (var i = 0; i < this.playerNodeList.length; i++) {
            var node = this.playerNodeList[i];
            if (node) {
                //给playernode节点发送事件
    ...
    */
    ...
    onDestroy: function onDestroy() {
        _mygolbal2.default.socket.clearEvent();
    }
    // update (dt) {},
