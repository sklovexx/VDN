...
cc.Class({
  ...
  onLoad: function onLoad() {
    ...
    //     }
    // }.bind(this))
    this.node.on("onOtherPlayerChuCard", function (data) {
      if (_this.accountid == data.accountid) {
        _this.bu_chu.active = false;
        if (data.accountid != _mygolbal2.default.playerData.accountID) {
          _this.destoryCard(data);
        }
      }
    }, this);
    this.node.on("onBuChu", function (data) {
      if (_this.accountid == data.accountid) {
        _this.bu_chu.active = true;
      }
    });
    this.node.on("room_waitready", function () {
      // if(this.accountid!=myglobal.playerData.accountID)
      try {
        _this.destoryAllCard();
        _this.masterIcon.active = false;
        _this.robnoIcon_Sp.active = false;
        _this.bu_chu.active = false;
        _this.clearCard();
      } catch (error) {
        console.warn(error);
    ...
    this.node.on("onCanChuCard", function (data) {
      if (data.accountid == _this.accountid) {
        _this.bu_chu.active = false;
      }
    });
    this.node.on("interval", function (data) {
      if (data.accountid == _this.accountid && _this.accountid != _mygolbal2.default.playerData.accountID) {
        _this.qiangdidzhu_node.active = true;
        _this.time_label.string = data.time;
      }
    });
    this.node.on("endInterval", function (data) {
      if (data.accountid == _this.accountid && _this.accountid != _mygolbal2.default.playerData.accountID) {
        _this.qiangdidzhu_node.active = false;
      }
    });
    this.node.on("onGameResult", function (data) {
      data.forEach(function (e) {
        if (e.account == _this.accountid) {
          _this.globalcount_label.string = e.amount;
        }
      });
    });
    this.node.on("onChangeHouseManage", function () {
      console.log(_this.accountid);
      if (_this.accountid == _mygolbal2.default.playerData.master_accountid) {
    ...
    this.node.on("show_bottom_card_event", function () {
      if (_this.accountid == _mygolbal2.default.playerData.master_accountid) {
        for (var i = 17; i < 20; i++) {
          ...
          _this.cardlist_node.push(card);
        }
      }
    });
    this.node.on("disconnect", function (data) {
      if (data.accountid == _this.accountid) {
        _this.offlineimage.active = true;
      }
    });
  },
  ...
  destoryCard: function destoryCard(data) {
    var tag = 0;
    for (var i = 0; i < data.cards.length; i++) {
      //删除牌组中的牌，一次去头，一次去尾
      if (tag == 0) {
        tag = 1;
        var index = parseInt(this.cardlist_node.length - 1);
  ...
  destoryAllCard: function destoryAllCard() {
    ...
    for (var i = 0; i < this.cardlist_node.length; i++) {
...
cc._RF.pop();
        ...
        //# sourceMappingURL=player_node.js.map
