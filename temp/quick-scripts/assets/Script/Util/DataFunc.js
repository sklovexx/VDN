(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/Script/Util/DataFunc.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '65af3WdcipBA4hi/Q3G/T+7', 'DataFunc', __filename);
// Script/Util/DataFunc.js
...
dataFunc.getMyDate = function (time) {
    var oDate = parseInt(time / 1000);
    var oHour = Math.floor(oDate / (60 * 60));
    var modulo = oDate % (60 * 60);
    //分钟
    var oMin = Math.floor(modulo / 60);
    //秒
    var oSen = modulo % 60;
    var oTime = dataFunc.addZero(oHour) + '小时' + dataFunc.addZero(oMin) + '分' + dataFunc.addZero(oSen) + '秒';
    return oTime;
};

//补零操作
dataFunc.addZero = function (num) {
    if (parseInt(num) < 10) {
        num = '0' + num;
    }
    return num;
...
dataFunc.queryValue = function (tableName, key, value) {
    var table = dataFunc.getTable(tableName);
    if (!table) {
        return null;
    }
...
dataFunc.loadConfigs = function (progressCb, callback) {
    ...
    dataFunc.arrTables.forEach(function (tableName, index) {
        cc.loader.loadRes("data/" + tableName, function (err, content) {
            ...
            addTable(tableName, content.text);
            if (callback) {
                currentLoad++;
