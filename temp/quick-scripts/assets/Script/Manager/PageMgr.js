(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/Script/Manager/PageMgr.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '9574fjLAz5Ex4lao+90mXLi', 'PageMgr', __filename);
// Script/Manager/PageMgr.js
...
cc.Class({
    ...
    closePage: function closePage(event, customData) {
        this.onClosePage(parseInt(customData));
    },
    openPage: function openPage(event, customData) {
    ...
    onClosePage: function onClosePage(num, tag) {
        if (isNaN(num)) return;
        var finish = function finish() {
            cc.find('Canvas/' + Global.pages[num]).active = false;
        };
        var action = cc.sequence(cc.scaleTo(0.2, 1, 0), cc.callFunc(finish, this));
        if (tag && tag == 1) {
    ...
    closeAllPages: function closeAllPages() {
        Global.pages.forEach(function (element) {
            cc.find('Canvas/' + element).active = false;
        });
    },
    showTips: function showTips(event, customData) {
    ...
    //顯示提示-豎屏
    showTipPage2: function showTipPage2(text) {
        this.Tips = cc.instantiate(this.TipsPage);
        this.Tips.angle = -90;
        this.Tips.parent = cc.find('Canvas');
        this.Tips.active = true;
        var action = cc.sequence(cc.moveBy(1, cc.v2(30, 0)), cc.fadeOut(0.1), cc.callFunc(this.closePage, this));
        this.Tips.runAction(action);
        this.Tips.getComponent('TipsPage').setText(text);
    },

    //顯示加載提示-橫屏
    showLoadingPage: function showLoadingPage(text) {
    ...
    //顯示加載提示-豎屏
    showLoadingPage2: function showLoadingPage2(text) {
        this.load = cc.instantiate(this.LoadingPage);
        this.load.angle = -90;
        this.load.parent = cc.find('Canvas');
        this.load.getChildByName("Text").getComponent(cc.Label).string = text;
    ...
    closeLoadingPage: function closeLoadingPage() {
        this.load.active = false;
    },
    backEcc: function backEcc() {
